...
const ProfileCard = ({ title, properties }: Props) => {
  ...
  return (
    <>
      <div className="mx-8 my-6 font-bold border-[1px] border-[#C6C4C4] w-[25%] pl-4 rounded-md">
        {title}
      </div>
          ...
          if (propertyKey === 'password') {
            return (
              <div key={propertyKey} className="flex mb-2 mt-1">
                <p className="capitalize text-[#A1ACB9] font-bold w-[17%]">
                  {propertyKey.replace(/_/g, ' ')}:
                </p>
                <Button
                  label="Change your password"
                  style="font-bold border-[1px] border-[#C6C4C4] px-3 rounded-md"
                />
              </div>
            );
