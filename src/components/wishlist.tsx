...
import { faMagnifyingGlass, faLocationDot, faTrash, faImage } from '@fortawesome/free-solid-svg-icons';
...
const Wishlist = () => {
  ...
  const handleSearch = () => {
    ...
    const item = updatedWishlist.filter((item) => item.wishlistItem.product.name.toLowerCase() === searchTerm.toLowerCase());
    if (item[0]) {
    ...
    <>
      <div className="text-left flex flex-col items-start w-[80%] m-auto lm:w-[70%] md:w-[80%] lg:w-[90%]">
        ...
        <div className="flex flex-col md:grid md:grid-cols-2 md:gap-7 lg:grid-cols-3">
                ...
                return (
                  <div className="relative h-[70vh] lg:h-[50vh] ll:h-[60vh] w-full mt-6 my-10 md:mb-6">
                    <div className="bg-[#F5F6F6] h-[60%] w-full rounded-lg flex items-center justify-center overflow-hidden ">
                      {product.wishlistItem.product.image[0] ? <img src={product.wishlistItem.product.image[0]} alt="" className="w-[100%] mt-6 object-cover object-top" /> : <FontAwesomeIcon icon={faImage} />}
                    </div>
                        ...
                        <div className="border-[1px] border-[#EA3A5B] rounded-full shadow-md border-opacity-80">
                          <FontAwesomeIcon
                            ...
                            onClick={() => {
                              ...
                              setUpdatedWishlist(updatedWishlist.filter((item) => item.wishlistItem.id !== product.wishlistItem.id));
                            }}
                          />
                        </div>
                      </div>
                  ...
                  <div className="border-[1px] border-[#EA3A5B] rounded-full shadow-md border-opacity-80">
                    <FontAwesomeIcon
                      ...
                      onClick={() => {
                        ...
                        setUpdatedWishlist(updatedWishlist.filter((item) => item.wishlistItem.id !== searchedItem.wishlistItem.id));
          ...
          //   <p>No such item in your wishlist</p>
          // </div>
          null}
        </div>
      </div>
